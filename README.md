...
| **임베딩 식별 (v2)** | ✅ 지금 작동 | 오픈소스 CLIP 로컬 실행. 작가의 다중 각도 사진을 인덱싱 → 관람객 사진과 코사인 매칭(≥0.85 확정), 애매하면 LLM 대조 폴백. 미등록 각도 사진 매칭 실측 통과 |
| **AI 인제스천 (v2)** | ✅ 지금 작동 | 등록 순간 자동: 사진들 임베딩 + VLM 캡션 + ai_profile(시각구성·촉각·안전·특징) 생성·저장. 설명은 안전 최우선→촉각(선천맹 색 대체) 순서로 재료 활용 |
| 작품 등록 API | ✅ 지금 작동 | `/uploads` `/artworks` + CORS. v2: 다중 사진(kind별)·영상·촉각/안전/위치/의도 수용, 하위호환 유지 |
| 작가 웹사이트 | ✅ 지금 작동 | Vite+React. v2 폼: 각도별 사진·영상 업로드, 접근성 필드(촬영 가이드 포함), 저장 후 AI 정리 미리보기 |
